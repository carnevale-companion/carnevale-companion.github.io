...
Carnevale measures distances in 3D, so the actual range to a lower target is the straight-line diagonal, not the horizontal gap. Enter the weapon's listed range and how far below the target is to find the maximum horizontal distance at which the weapon still reaches.
